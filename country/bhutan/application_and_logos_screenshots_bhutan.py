...
'''Spliting text based on delimiter'''
...
def remove_extra_headings_bhutan(content_data, texts_to_remove, texts_to_remove_start,delimiter):
    corrected_data = []
    for page_text in content_data:
        text = page_text['text']
...
def extract_and_segment_applications_bhutan(pdf_file_path):
    # Define a regular expression pattern to match specific text sections
    pattern = re.compile(r"\(210\)[\s\S]*?(?=\(210\)|$)")  #BHUTAN

    # input_file_path = r'input_pdf\BT20231003-110.pdf'
    # # Extracting just the filename
    # pdf_file_name = os.path.basename(input_file_path)

    # Path to the PDF file
    # pdf_file_path = 'BT20230215-106.pdf'
    # input_file_path = pdf_file_path
    ...
    #             new_pn_matching_PDF = pn + 1
        ...
        section_data[section_key]["content_data"] = remove_extra_headings_bhutan(section_data[section_key]["content_data"], texts_to_remove, texts_to_remove_start, delimiter)
...
def extract_and_process_images_bhutan(json_data, pdf_file, output_folder,output_parent_folder, resolution=200):
...
def extract_logos_bhutan(pdf_file_path, json_path, output_folder, output_parent_folder):
    # Read JSON data
...
def image_processing_Bhutan(pdf_file_path):
    # Step 1: Extract and segment applications
    result_file_path = extract_and_segment_applications_bhutan(pdf_file_path)
    ...
    define_coordinates_from_pdf_for_application_screenshots_bhutan(pdf_file_path, extracted_text_json_path)

    # FOLDER WHERE THE IMAGES ARE SAVED
    output_parent_folder = 'Bhutan_images'
    ...
    extract_and_process_images_bhutan(json_data, pdf_file_path, output_folder_path, output_parent_folder)

    # Step 4: Extract logos
    json_path = f'output_{pdf_file_path}.json'
    output_folder_logos = f"logo_images_{pdf_file_path}"
    extract_logos_bhutan(pdf_file_path, json_path, output_folder_logos, output_parent_folder)
...
if __name__ == "__main__":
    ...
    pdf_path =  'BT20231003-110.pdf'
    image_processing_Bhutan(pdf_path)