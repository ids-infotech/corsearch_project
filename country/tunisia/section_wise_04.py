...
def remove_tradeMark_numbers(input_json_file, output_json_file, pdf_file_path):
    ...
    fields_to_extract = {
        "Applications": ["MARQUES ETRANGERES", "PUBLICATION DES MARQUES NATIONALES"],
        "Madrid Applications": ["MARQUES INTERNATIONALES"],
    }

    # Iterate over the specified fields
    for section_type, field_names in fields_to_extract.items():
        # Create a section dictionary for the current field
        section_data = {"title": section_type, "tradeMarks": []}
...
pdf_file_path = r'input_pdf\TN20230623-252.pdf'
