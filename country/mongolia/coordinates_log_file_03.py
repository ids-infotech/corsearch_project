...
from final_structure_02 import process_bulk_trademarks, json_file_path, pdf_file_path

def update_structure_with_clippings_MONGOLIA(input_structure_file, input_clippings_file, output_structure_file, log_file):
    ...
    nested_keys = headings = ["Registration", "MADRID REGISTRATION"]  # Add your actual nested keys
    ...
    for nested_key in nested_keys:
        ...
        if nested_key in structure_data:
            ...
            for trademark_name, trademark_data in nested_trademark_data.items():
                ...
                registration_number = trademark_data[0].get("registrationNumber") if trademark_data else None

                # Increment total_entries counter
                total_entries += 1

                # Check if the registration number exists in clippings_data
                if registration_number in clippings_data:
                    # Extract content data from clippings_data
                    content_data = clippings_data[registration_number].get("content_data", [])
                    ...
                    log_info = f"Registration Number: {registration_number}"
