    ...
    for section_config in config["applicableFor"]["sections"]:
        if section_config["title"] == "Registrations":
            ...
            for match in registration_pattern.finditer(pdf_text):
                ...
                owner_address_match = match_732_address.group(2) if match_732_address else ""

              
                owner_name_parts = owner_name.split(",", 1)
                owner_name = owner_name_parts[0].strip() if owner_name_parts else ""

                if len(owner_name_parts) > 1:
                    owner_address = f"{owner_name_parts[1].strip()} {owner_address_match}".strip()
                else:
                    owner_address = owner_address_match
...
# Adjust the extracted data
for entry in result:
    for trademark in entry["trademarks"]:
        for owner in trademark["owners"]:
            # Split name and address
            name_parts = owner["name"].split(",", 1)
            
            # Update name and address fields
            owner["name"] = name_parts[0].strip() if name_parts else ""
            owner["address"] = (name_parts[1] + " " + owner["address"]).strip() if len(name_parts) > 1 else owner["address"]

# Save data to a JSON file
with open("own2.json", "w", encoding='utf-8') as output_file:
