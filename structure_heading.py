    ...
    for entry in trademark_data:
        if entry.startswith('(111)'):
            ...
            if len(entry.split(':')) > 1:
                ...
                next_line_value = ": " + trademark_data[trademark_data.index(entry) + 1].strip()import json
import re

def extract_nice_classes_and_goods(values):
    ...
    for line in values:
        ...
        if match_class:
            if current_class is not None:
                output_data["classifications"].append({"niceClasses": int(current_class), "goodServiceDescription": current_goods.strip()})
            current_class = match_class.group(1)
            current_goods = match_class.group(2)
        else:
            current_goods += " " + line.strip()
...
def process_trademark_section(trademark_data):
    ...
    for entry in trademark_data:
        if entry.startswith('(111)'):
            ...
            nice_classes_and_goods = extract_nice_classes_and_goods(trademark_data[trademark_data.index(entry):])
            filtered_trademark_data["niceClassesAndGoods"] = nice_classes_and_goods["classifications"] 
